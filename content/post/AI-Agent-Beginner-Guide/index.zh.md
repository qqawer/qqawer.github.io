...
{{< figure src="/post/ai-agent-beginner-guide/tool-interface-xiaohei.png" alt="小黑在含糊的处理按钮和参数明确的工具接口之间做选择" caption="工具不是越万能越好：名称、参数和边界越明确，模型越不容易接错线。" >}}
...
{{< figure src="/post/ai-agent-beginner-guide/memory-retrieval-xiaohei.png" alt="小黑从大量记忆抽屉中检索少量相关、近期和重要的信息" caption="长期记忆的价值不在于存得多，而在于把相关、近期、重要的信息送回当前上下文。" >}}
...
{{< figure src="/post/ai-agent-beginner-guide/single-vs-multi-agent-xiaohei.png" alt="清晰的单 Agent 工作台与过早拆分造成的多 Agent 交接损耗" caption="多 Agent 是隔离复杂度的手段，不是默认升级：拆得过早，简单任务也会产生交接损耗。" >}}
